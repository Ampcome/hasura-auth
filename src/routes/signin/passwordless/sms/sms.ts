...
> = async (req, res) => {
  ...
  // block-listed phone numbers cannot proceed
  // logger.info(`[sms] signin start phoneNumber=${phoneNumber}`);
  const phoneBlock = await getActivePhoneBlock(phoneNumber);
  // logger.info(`[sms] phoneBlock=${phoneBlock ? 'BLOCKED' : 'allowed'}`);
  if (phoneBlock) {
    return sendError(res, 'phone-number-blocked');
  }
  ...
  const decode: any = jwt.verify(
    passwordless_token,
    ENV.HASURA_GRAPHQL_JWT_SECRET.key
  );
  ...
  if (metadata?.src === 'otpless') {
    ...
    const otplessResponse = await signInOtplessHandler(phoneNumber, {
      defaultRole,
      allowedRoles,
      displayName,
      locale,
